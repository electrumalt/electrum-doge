...
register_command('createmultisig',       2, 2, False, True,  False, 'similar to ixcoind\'s command')
register_command('createrawtransaction', 2, 2, False, True,  False, 'similar to ixcoind\'s command')
register_command('deseed',               0, 0, False, True,  False, 'Remove seed from wallet, creating a seedless, watching-only wallet.')
register_command('decoderawtransaction', 1, 1, False, False, False, 'similar to ixcoind\'s command')
register_command('getprivatekeys',       1, 1, False, True,  True,  'Get the private keys of a given address', 'getprivatekeys <ixcoin address>')
...
register_command('getpubkeys',           1, 1, False, True,  False, 'Return the public keys for a wallet address', 'getpubkeys <ixcoin address>')
...
class Commands:
    ...
    def getversion(self):
        import electrum_ixc as electrum  # Needs to stay here to prevent ciruclar imports
    ...
    def _mktx(self, outputs, fee = None, change_addr = None, domain = None):
        for to_address, amount in outputs:
            if not is_valid(to_address):
                raise Exception("Invalid Ixcoin address", to_address)

        if change_addr:
            if not is_valid(change_addr):
                raise Exception("Invalid Ixcoin address", change_addr)

        if domain is not None:
            for addr in domain:
                if not is_valid(addr):
                    raise Exception("invalid Ixcoin address", addr)
